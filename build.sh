...
cd build_release
cmake .. -DCMAKE_BUILD_TYPE=Release ${vcpkg_define} ${vcpkg_triplet_define} ${additional_defines} ${additional_build_setup} -DCUDNN_INCLUDE_DIR="/cm/shared/package/cuDNN/cuda10.1/7.6.4/cudnn/cuda/" -DCUDNN_LIBRARY="/cm/shared/package/cuDNN/cuda10.1/7.6.4/cudnn/cuda/lib64/libcudnn.so"
cmake --build . --target install -- -j${number_of_build_workers}
#cmake --build . --target install --parallel ${number_of_build_workers}  #valid only for CMake 3.12+
rm -f DarknetConfig.cmake
rm -f DarknetConfigVersion.cmake
